...
| `authentication.statelessAccessTokens`    | `true`                                         | When enabled, access and refresh tokens are verified from their JWT signature and expiration without using the `sessions` table. This removes session database writes during login and refresh, but logout cannot revoke already issued tokens server-side. Token invalidation then relies on token expiration or JWT secret rotation. |
| `authentication.database`                 | SQLite `db`                                    | DB0-compatible database used for users, sessions when stateful auth is enabled, and email verification records.                                                                                                                                                                                                                        |
